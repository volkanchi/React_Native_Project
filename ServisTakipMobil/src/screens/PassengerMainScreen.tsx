...
export default function PassengerMainScreen({ onNavigateToLiveTracking, onLogout }: PassengerMainScreenProps) {
  ...
  const handleJoinRoute = async () => {
    ...
    const result = await routeService.joinRoute({ routeCode: routeCode.trim() }, token);
      ...
      <View style={styles.bottomSheet}>
        ...
        <ScrollView style={styles.routeList} showsVerticalScrollIndicator={false}>
          {fetchingRoutes ? (
            ...
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>Henüz bir servise kayıtlı değilsiniz. Aşağıdan kod ile katılabilirsiniz.</Text>
            </View>
          ) : (
            myRoutes.map((route) => (
              <View key={route.id} style={styles.routeCard}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.routeDriver}>Şoför: {route.driverName}</Text>
        ...
        <View style={styles.joinArea}>
          ...
          <View style={styles.joinRow}>
            ...
            />
            <TouchableOpacity style={styles.joinButton} onPress={handleJoinRoute} disabled={loading}>
              {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.joinButtonText}>Katıl</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  topBar: { position: 'absolute', top: 50, left: 20, right: 20, flexDirection: 'row', justifyContent: 'space-between', zIndex: 2 },
  topBarLeft: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.95)', padding: 10, borderRadius: 12, gap: 8 },
  ...
  bottomSheet: { position: 'absolute', bottom: 0, left: 0, right: 0, height: '65%', backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 10, elevation: 15 },
  sheetTitle: { fontSize: 20, fontWeight: '800', color: '#111827', marginBottom: 16 },
  
  routeList: { flex: 1 },
  emptyBox: { alignItems: 'center', justifyContent: 'center', padding: 20, backgroundColor: '#F3F4F6', borderRadius: 16, marginTop: 10 },
  emptyText: { textAlign: 'center', color: '#6B7280', marginTop: 10, fontSize: 14, lineHeight: 20 },
  
  routeCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', padding: 16, borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: '#E2E8F0' },
  ...
  watchButton: { flexDirection: 'row', backgroundColor: '#2563EB', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12, alignItems: 'center', gap: 6 },
  watchButtonText: { color: '#fff', fontWeight: 'bold', fontSize: 14 },
  
  joinArea: { marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#E5E7EB' },
  joinTitle: { fontSize: 14, fontWeight: '600', color: '#475569', marginBottom: 10 },
  joinRow: { flexDirection: 'row', gap: 10 },
  input: { flex: 1, backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#CBD5E1', borderRadius: 12, paddingHorizontal: 14, height: 48, color: '#111827' },
  joinButton: { backgroundColor: '#10B981', borderRadius: 12, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 20 },
