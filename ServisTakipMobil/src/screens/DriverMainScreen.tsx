...
export default function DriverMainScreen({ onLogout }: DriverMainScreenProps) {
  ...
  const handleStartTrip = async () => {
    ...
    try {
      ...
      routeIdRef.current = routeIdRef.current || 'pending-route-id';
      setRouteData({
        routeId: routeIdRef.current,
        name: 'Aktif rota bilgisi API’den gelecek',
        number: 'API',
        routeCode: 'API-ROTA-KODU',
        plate: '',
        vehicleModel: '',
        capacity: 0,
        stops: [],
      ...
      >
        ...
        {routeStops.map((stop, i) => (
          <Marker
            ...
            pinColor={i < currentStopIndex ? '#94A3B8' : i === currentStopIndex ? '#1E4ED8' : '#CBD5E1'}
          />
        ))}
        {driverCoord && (
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.headerRow}>
            <View>
              ...
              <Text style={styles.title}>{routeData?.name || 'Gerçek rota verisi bekleniyor'}</Text>
            </View>
            <View style={styles.capacityBox}>
              <Text style={[styles.capacityValue, { color: occupancyPct > 0.8 ? '#EF4444' : '#22C55E' }]}>{occupancy}</Text>
          ...
          <View style={styles.summaryCard}>
            ...
            <View style={styles.summaryItem}>
              ...
              <Text style={styles.summaryValue}>{routeData?.vehicleModel || 'API’den gelecek'}</Text>
            </View>
          </View>
          ...
          <View style={styles.controlsRow}>
            ...
            >
              <Text style={styles.primaryButtonText}>{connecting ? 'Bağlanıyor...' : tripActive ? 'Sefer açık' : 'Seferi başlat'}</Text>
            </TouchableOpacity>
              ...
              {routeStops.map((stop, i) => (
                ...
                >
                  <View style={styles.stopMarkerContainer}>
                    <View style={[styles.stopMarker, i < currentStopIndex && styles.stopMarkerPassed, i === currentStopIndex && styles.stopMarkerCurrent]} />
          ...
          {!routeData && (
            <View style={styles.emptyStateBox}>
              <Text style={styles.emptyStateText}>
                Gerçek rota verisi API’den gelene kadar bu ekran bekleme modunda kalır.
              </Text>
            </View>
          )}
        </ScrollView>
